...
public class CopyFunctionWithDependenciesAction extends AnAction {
    private static final Logger LOG = Logger.getInstance(CopyFunctionWithDependenciesAction.class);
    private final MarkdownFormatter formatter = new MarkdownFormatter();
    private FunctionExtractor extractor;
    private boolean debug = false;

    public CopyFunctionWithDependenciesAction() {
        super();
    ...
    public void update(@NotNull AnActionEvent e) {
        if (debug) {
            LOG.info("CopyFunctionWithDependenciesAction.update called");
        }
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        if (debug) {
            LOG.info("CopyFunctionWithDependenciesAction.actionPerformed called");
        }
        ...
        try {
            if (selectionModel.hasSelection()) {
                ...
                if (selectedText == null || selectedText.trim().isEmpty()) {
                    CopyUtil.showErrorHint(project, "No text selected");
                    return;
                }

                if (debug) {
                    LOG.info("Extracting from selection: " + selectedText.substring(0, Math.min(50, selectedText.length())) + "...");
                }

                extractedCode = getExtractor(project).extractFunctionWithDependencies(psiFile, selectionModel.getSelectionStart());
            } else {
                int offset = editor.getCaretModel().getOffset();
                if (debug) {
                    LOG.info("Extracting from cursor position: " + offset);
                }

                extractedCode = getExtractor(project).extractFunctionWithDependencies(psiFile, offset);
            }
            ...
            String markdownCode = formatter.formatFileContent(project, virtualFile, extractedCode);
            CopyUtil.copyToClipboardWithNotification(markdownCode, project);

        } catch (Exception ex) {
            LOG.error("Error extracting function", ex);
            CopyUtil.showErrorHint(project, "Error extracting function: " + ex.getMessage());
        }
    }
